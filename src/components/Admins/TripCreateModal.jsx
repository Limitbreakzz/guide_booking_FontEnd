...
const TripCreateModal = ({ isOpen, onClose, onUpdate }) => {
  ...
  const fetchGuides = async () => {
    try {
      ...
      setGuides(Array.isArray(res.data.data) ? res.data.data : []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/trips`, form, {
        headers: { 
          Authorization: `Bearer ${token}`,
          "Content-Type": "multipart/form-data"
        }
      });
      setSuccess("สร้างทริปใหม่สำเร็จ!");
      setFormData({
        name: "",
        price: "",
        description: "",
        ...
        <>
          ...
          >
            ...
            <div className="sticky top-0 bg-white border-b border-slate-200 px-8 py-6 flex items-center justify-between">
              <h2 className="text-2xl font-black text-[#37101A]">สร้างทริปใหม่</h2>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="px-8 py-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Guide */}
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">ไกด์ผู้ดูแล</label>
                  <select
                    name="guideId"
